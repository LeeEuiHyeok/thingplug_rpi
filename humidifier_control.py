...
def fun_getLatestData(thingplug, node_id, container, enable_log):
    ...
    try:
        ...
        output_data = data + ' ,' + str(beepsound) + '\r\n'
        # output_data = data + '\r\n'
        print(output_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pt', '--period_time', type=int, help='Get Latest Data Period Time(Default:0,One Time)',
                        required=False)
    ...
    thingplug = ThingPlug_Py3.ThingPlug(THINGPLUG_HOST, THINGPLUG_PORT)
    thingplug.login(args.user_id, args.user_pw)

    thingplug.setAppEui(args.app_eui)

    if PERIOD_TIME == 0:
        status, data, lt_time = thingplug.getLatestData("mobisys_humidity", args.container)
        current_time = str(datetime.datetime.now())
        ...
        while True:
            ...
            param = (thingplug, "mobisys_humidity", args.container, args.enable_log)
